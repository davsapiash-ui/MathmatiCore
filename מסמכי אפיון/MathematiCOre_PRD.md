# מסמך דרישות מוצר מפורט (Master PRD v3.5)
## פלטפורמת הלמידה ההיברידית "מתמטיקאור" (MathematiCOre)
**מספר גרסה:** Version 3.5  
...
2. **רכיבי ממשק המשתמש והעיצוב החזותי**: מנוע גרפי מבוסס HTML5 Canvas / WebGL להצגת הבלוקים הדיגיטליים ב-60fps וההרחבת שטחי השמיטה ב-30px.
...
3. **פלט**: 1. הצגת כרטיס חניכה בצד ללא פופ-אפ. 2. שינוי עיצוב כפתורים (מסיח שגוי = אדום, שאר הלחצנים = 40% שקיפות, סמן חסימה not-allowed). 3. חיווי שעון חול דיגיטלי/סרגל התקדמות. 4. הצגת הנחיה שקטה על נעילה ושימוש בביטול פעולה.
...
2. **עיבוד**: 1. חישוב מדד ההתמדה (Persistence Index): `(U / (U + E + G)) * 100`. אם המכנה 0, המדד מוגדר כ-100%.
...
2. **רכיבי ממשק המשתמש והעיצוב החזותי**: גריד קבוע של 12 משבצות (3x4) המייצגות את 12 תלמידי הפיילוט, הממוינות לפי מזהה מספרי 1-12 בלבד ללא שמות (`תלמיד 1` עד `תלמיד 12`), עם עדכון צבעי רקע בלבד וללא חלונות קופצים (Popups).
3. **לוגיקת אינטראקציה ומכונת המצבים**: עדכון צבעי רקע בלבד (ירוק, צהוב, אדום, אפור) במענה לנתוני טלמטריה בזמן אמת (זמן תגובה 1000ms) ללא הזזת אלמנטים או פופ-אפים.
...
1. **מטרת המודול הפדגוגית והטכנולוגית**: ערוץ תקשורת ואבחון פדגוגי עם פילטר PII דו-שכבתי קשיח: חסימה והתראה בזמן אמת בצד הלקוח באמצעות ביטויים רגולריים (Client-side Regex), והחלפה אוטומטית למזהים אנונימיים בצד השרת (Server-side Anonymization) לפני השמירה ב-Firestore.
2. **רכיבי ממשק המשתמש והעיצוב החזותי**: ממשק תקשורת פדגוגית משולב צדדי ודשבורד המאפשר שיח ישיר בין מורה למנהל ולתלמיד עם בועות שיחה מיושרות ומחווני התראה.
3. **לוגיקת אינטראקציה ומכונת המצבים**: Client-side Regex חוסם שליחה עם שמות. Cloud Function מריץ anonymizerService שממפה שמות ל-student_id לפני Commit ל-Firestore / Realtime Database.
4. **תרחישי קצה וטיפול בשגיאות ממשק**: זיהוי פרטים מזהים מציג התראה מקומית עדינה ללא שליחת הנתונים לשרת.
5. **דרישות מרווחים נגישות וביצועים**: סנכרון מהיר ורציף תוך פחות מ-100ms.
6. **מבנה נתונים ושדות בסיס הנתונים**: message thread מסוג Array / Realtime Database nodes תחת `chat_messages`.
7. **לוגיקת שחזור וריענון דפדפן**: טעינה אוטומטית והאזנה רציפה עם `initSync`.
8. **מניעת לולאות עדכון בזמן אמת**: שימוש במנויי Snapshot מבוקרים.
...
1. **מטרת המודול הפדגוגית והטכנולוגית**: קלט: 1. מדד שליטה מופשטת ממפגש 8. 2. מפת פערי ידע מהמטריקס האבחוני תוך ניתוח והשוואה בין קבוצת הניסוי לקבוצת הביקורת.
