...
1. **דרישות:** דרישות המערכת מחייבות יצירת שער כניסה מאובטח השומר על פרטיות מוחלטת של הלומד על בסיס מודל ורה דיגיטלי בלבד ומבטיח תחילת עבודה ללא חסמים רגשיים. הממשק כולל כפתור כניסת מורים באמצעות מערכת הזדהות גוגל בלבד המבוסס על התאמה מדויקת מול אוסף מורים מורשים בבסיס הנתונים פיירבייס. הכתובות המורשות היחידות להזדהות כמורה בשיטת התאמה מדויקת הן davidsep@edu-haifa.org.il וכן 1002220159@edu-haifa.org.il. כל כתובת דואר אלקטרוני אחרת תיחסם באופן גורף ומיידי. אפיון המקלדת הוירטואלית הקומפקטית של התלמידים מוגדר כרכיב קבוע על גבי מסך ההזדהות במבנה של שלוש עמודות וארבע שורות המכיל את הספרות אפס עד תשע כפתור מחיקה וכפתור אישור. גודל כל מקש במקלדת יהיה ארבעים ושמונה פיקסלים לפחות עם מרווח בטיחות של שנים עשר פיקסלים בין מקש למקש. במצבי שגיאת הזדהות המערכת תבצע רטט חזותי עדין של תיבת הקלט למשך שלוש מאות מילישניות תרוקן את שדה הקוד באופן אוטומטי ותחזיר את מיקוד המקלדת לתחילת השדה.
2. **אפיון:** המערכת פועלת במצב Idle עד לזיהוי קליק, ועוברת למצב Auth Redirect באמצעות Google SSO עבור מורים, או לתהליך מדורג וחד-כיווני עבור תלמידים (בית ספר -> כיתה -> מזהה תלמיד 1-12 -> קוד גישה 10203040). שמירת student ID מסוג Integer בטווח 1 עד 12 ב-Local Storage. זמן תגובה לאימות פחות מ-200ms, שטחי הקלקה בגודל 48px לפחות, ומנגנון Throttle של 500ms על כפתורי הבחירה.
3. **Strict Bilingual Developer Instructions:** Eradiate all CRA terminology and physical object references. All design specifications and code structures must implement the VRA model only. No placeholders or hints allowed on input elements. Keep student profiles strictly anonymous using numeric IDs 1 to 12 with zero persona names in UI. Teachers login must rely exclusively on Google SSO verified via exact match query against the authorizedTeachers Firestore collection, with davidsep@edu-haifa.org.il as the single default allowed email address and zero hardcoded email lists in the source code. Students login flow is strictly sequential: School Dropdown selecting "בית ספר ביקורת" -> Class Dropdown selecting "המבקרים" -> Compact Numeric Keypad for ID selection (1 to 12) -> Password input accepting strictly the value 10203040.
