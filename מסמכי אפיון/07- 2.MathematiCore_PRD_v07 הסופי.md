גרסה 7.2 | עודכן 30 באוגוסט 2026, 20:41
...
* **Firebase Realtime Database (RTDB)**, שכבת זמן-אמת וסנכרון פעילות שוטפת: users/students/{studentId} (סנכרון מצב מרחב העבודה ו-Presence של התלמיד בזמן אמת: lastAction, activeBranch, isOnline, lastPing, תרגיל נוכחי, helpRequested (boolean)), chat\_messages/{roomId} (תקשורת צ'אט מהירה בזמן אמת בין תלמיד למורה), active\_class\_session (שידור מצב סשן פעיל ומצב מקרן כיתתי: projector\_mode).
...
* **אוסף קטלוג תוכנית הלימודים ב-Firestore (Curriculum Catalog Collection)**, שכבת תכני הלימוד והמשימות: קטלוג תוכנית הלימודים מנוהל כאוסף קנוני ב-Firestore המאפשר עדכונים אסינכרוניים של תבניות התרגילים (Exercise Templates). הקטלוג נטען מהשרת ונשמר במטמון מקומי ב-IndexedDB בצד הלקוח כדי להבטיח ביצועים גבוהים, טעינה מיידית של משימות 1–8 ועמידות מלאה במצב לא מקוון (Offline Resilience).
...
**בית המספרים כגשר ייצוגי הוליסטי (Representational Bridge):** לוח בית המספרים והקנבס אינם עומדים בפני עצמם ואינם מהווים ארגז חול מנותק במהלך פתרון תרגיל. הם משמשים כבבואה ייצוגית מדויקת המחוברת בקשר ישיר לשלב הפתרון באלגוריתם, לטור הפעיל ולפעולה המתמטית של התרגיל כולו.
...
* **כלי עזר ממשקי \- פח האשפה (Trash Can):** על גבי מסך העבודה יוצב באופן קבוע כלי עזר ממשקי המכונה "פח האשפה". מנוע הגרירה והחישוב תומך בשתי פעולות מחיקה ואיפוס: גרירת לבנת דינס בודדת אל פח האשפה מוחקת את הלבנה מיידית מהקנבס ומעדכנת את מצב המערכת בהתאם; לחיצה ישירה על כפתור פח האשפה מאפסת ומנקה לחלוטין את כלל הלבנים הווירטואליות הקיימות במרחב העבודה הנוכחי. פעולות המחיקה והאיפוס באמצעות פח האשפה מתועדות כאירועי טלמטריה תקניים ב-Zustand ומסונכרנות לשרת, וכמו מנגנון ה-Undo, מוחרגות לחלוטין ממדדי השגיאות והשחיקה.

**הנחיות פיתוח נוקשות (Strict Developer Instructions)**

*Use HTML5 Canvas/WebGL for block manipulation running steadily at 60fps. Implement magnetic snap physics for snapping blocks into standard column layouts. Undo actions must be excluded from mistake telemetry tallies. Implement a permanent interface tool called 'Trash Can' (פח האשפה); the engine must support dragging a single Dienes block to the trash can for immediate workspace deletion, as well as clicking the trash icon directly to clear all virtual blocks from the current workspace. Record these trash actions as valid telemetry events in Zustand and sync them to the server, while explicitly excluding them from student mistake and fatigue metrics. Implement two equivalent decomposition triggers, both active in every session where the canvas renders: (a) single click on a block, which immediately breaks it into ten blocks of the adjacent lower value, and (b) dragging the block into the adjacent column to its right. Both paths must emit the identical REGROUPING\_SUCCESS event and run the identical animation. Implement composition through exactly one path: when a column accumulates ten or more blocks, render a dedicated grouping button at the top of that column; clicking it merges ten blocks into one block in the adjacent column to the left. Reject leftward drag as a grouping mechanism.*
...
**עמעום וסגירה (החלטת בעל המוצר, 7 בספטמבר 2026):** הלוח נטען באנימציית עמעום רכה של 2 שניות, ונשאר על המסך עד שהלומד סוגר אותו בכפתור X. שום דבר אינו סוגר אותו אוטומטית. (ההיעלמות האוטומטית 3 שניות אחרי ספרה נכונה, שהוצעה במסמך 05, נבנתה ובוטלה: לוח שנעלם באמצע תרגיל מפתיע את הלומד שזקוק לו.)
...
בעת זיהוי קושי, מופעל כרטיס החניכה הצידי (Socratic Card) במגירה נשלפת (Side Drawer). בעת הפעלת הטריגר, המערכת דוגמת Snapshot הוליסטי מלא של סביבת הלמידה: (א) נתוני התרגיל והטור הפעיל, (ב) כמויות הלבנים בבית המספרים ומצב הפריטה/הקבצה, (ג) שלב הפתרון של התלמיד, הספרות שהוזנו והספרות בעיגולי הזיכרון. 

בעת בחירת תשובה שגויה בתוך כרטיס החניכה, המערכת מפעילה נעילה שקטה של לחצני המענה בכרטיס בלבד למשך 30 שניות (pointer-events: none בתוספת אינדיקטור שעון חול עדין). בזמן נעילת כרטיס החניכה, מרחב הלבנים הדיגיטליות, המקלדת, וכפתור ה-Undo נשארים פעילים וחופשיים לחלוטין לחקירה.

**הנחיות פיתוח נוקשות (Strict Developer Instructions)**

*Implement the Socratic card in a dedicated side drawer without using popup dialogs or blocking modals. Trigger Socratic card activation strictly upon: (a) 45 seconds of continuous column hesitation, or (b) 4 consecutive digit deletions/errors in the active column, or (c) in session 8 only, 3 consecutive undo actions within a single exercise. These three conditions correspond exactly to trigger\_reason: 'hesitation\_45s' | 'consecutive\_errors\_4' | 'consecutive\_undos\_3' in SocraticCardShownDetails (Appendix A §3). Condition (c) must never fire in sessions 1 through 7\. In session 2 the Socratic card is disabled entirely and none of the three conditions may fire. Upon incorrect answer selection inside the Socratic card, immediately apply a 30-second pointer-events-disabled block exclusively to the card buttons with a serene hourglass indicator. Ensure Dienes blocks canvas interaction and the undo button remain fully functional and accessible. Send workspace telemetry JSON encapsulating the complete holistic pedagogical triad (exercise definition, live block counts, and student step progression) to initiate backend AI analysis.*

# **מודול 13: ממשק החונך הדיגיטלי ומנוע החניכה** *(Socratic AI Engine & Gemini API Contract)*

## **א. מנוע האבחון ועקרון השילוש הפדגוגי ההוליסטי**

מנוע הבינה המלאכותית מקבל כקלט את מטען הטלמטריה ומצב מרחב העבודה המלא של התלמיד. 

**חוק ברזל פדגוגי — עקרון השילוש הפדגוגי ההוליסטי (Holistic Pedagogical Triad):**
חל איסור מוחלט על מנוע ה-AI או על מנגנון הרמזים הסטטיים להתייחס לבית המספרים או לקנבס הלבנים במנותק מהתרגיל המתמטי ומשלב הפתרון של התלמיד. כל ניסוח של שאלה מנחה, מסיח או משוב חייב לארוג יחד שלושה מרכיבים:
1. **התרגיל והאלגוריתם:** הפעולה המתמטית (חיבור/חיסור), המספרים בתרגיל (למשל: $425 - 162$), והטור הפעיל כעת (למשל: חיסור $2 - 6$ בעשרות).
2. **המצב הייצוגי בבית המספרים:** כמות הלבנים המדויקת בכל טור בקנבס, והאם בוצעה כבר פעולת פריטה/הקבצה בלבנים (למשל: האם קיימות 2 עשרות ו-4 מאות, או שנפרטה מאה אחת ל-10 עשרות).
3. **שלב הביצוע והיסטוריית הקלט של התלמיד:** אילו טורים כבר נפתרו נכון (למשל: יחידות $5 - 2 = 3$), מה הוזן בתיבות הקלט ובעיגולי הזיכרון, ומה סיבת הטריגר (השהיה / שגיאות חוזרות).

המנוע מסווג את מקור הקושי לאחת משלוש קטגוריות מחייבות:
1. **calculation (טעויות עובדתיות בחישוב הבסיסי):** התלמיד מבין את המבנה והאלגוריתם אך שגה בעובדת חיבור/חיסור בסיסית.
2. **procedural (טעויות מיומנות רכיב / סדר האלגוריתם):** התלמיד מדלג על שלב, מתחיל מטור שגוי, שוכח לעדכן את עיגול הזיכרון, או מחסיר מלמטה למעלה במקום לפרוט.
3. **conceptual (טעויות אסטרטגיה / חוסר הבנה מושגית של המבנה העשרוני):** התלמיד אינו מבין את משמעות ערך המקום, מנסה לרשום מספר דו-ספרתי במשבצת בודדת, או מוחק בלוקים מבלי לשמר את הכמות הכוללת.

בהתאם לסיווג ולשילוש הפדגוגי, המנוע מנסח שאלה מנחה סוקרטית אחת קצרה ושלושה מסיחים סגורים ומותאמים.

**כללי תחביר עברי, דקדוק וטרמינולוגיה פדגוגית תקנית:**
כל טקסט המנוסח על ידי מנוע ה-AI או מוגש ברמזים המובנים חייב לעמוד בכללים נוקשים:
1. **דקדוק והתאמת מין ומספר:** עברית תקנית ומדויקת, ללא שגיאות התאמה (למשל: 4 מאות, 2 עשרות, 5 יחידות, 10 עשרות).
2. **התאמה התפתחותית לגילאי כיתה ג' (גילאי 7–9):** משפטים קצרים, בהירים, זורמים ומעצימים, ללא סרבול לשוני וללא עומס קוגניטיבי.
3. **טרמינולוגיה רשמית בהלימה לתוכנית הלימודים של משרד החינוך:**
   - בחיסור: "פריטה" (ולא "שבירה" או "הלוואה").
   - בחיבור: "קיבוץ" / "הקבצה" / "צירוף עשר" (ולא "נשיאה").
   - מרחב העבודה: "בית המספרים", "טור היחידות", "טור העשרות", "טור המאות".
   - כלי עזר: "עיגולי הזיכרון", "פח האשפה".
4. **טון מעודד ונטול שיפוט (UDL):** המשובים למסיחים שגויים מנוסחים כרמז מנחה מכוון (למשל: "רמז: בחיסור אנו גורעים רק מהכמות הקיימת, ולא מחסירים הפוך") ללא שיפוט, ללא נזיפה וללא תיוג.

**חוקי ברזל למנוע ה-AI:**
1. איסור מוחלט לחשוף את התשובה המספרית הסופית או לפתור עבור התלמיד.
2. איסור מוחלט על שאלות גנריות ומנותקות ("ראיתי בלוח X בלוקים מה הצעד הבא?"). השאלה חייבת לקשור את הבלוקים לתרגיל ולשלב הפתרון.
3. איסור שימוש במונחי עזרים פיזיים שאינם קיימים בממשק הדיגיטלי.
4. במקרה של כשל תקשורת, Timeout או חריגה מסכמת ה-JSON, המערכת מגישה רמז סוקרטי סטטי מובנה מראש העומד במלוא עקרונות השילוש הפדגוגי.

**שמירת הסיווג האבחוני:** המנוע מחזיר את הסיווג בשדה `error_category` בתשובת ה-API, והמערכת שומרת אותו כשדה בתוך אירוע הטלמטריה `SOCRATIC_CARD_SHOWN`. חל איסור מוחלט להשליך את הסיווג. סיווג זה מזין ישירות את האות הקוגניטיבי ברדאר הפדגוגי (מודול 18) ואת הדוח המסכם (מודול 23).
...
*Enforce rigid JSON schema validation for the Gemini API exchange matching GeminiSocraticRequest and GeminiSocraticResponse. Enforce the Holistic Pedagogical Triad across all AI prompts and fallback hints: synthesize (1) Exercise Context (operands, operation, active column), (2) Live Board State (Dienes block counts, canvas decomposition status), and (3) Student Step Progress (completed columns, memory circles, hesitation/error trigger). Strictly prohibit isolated or detached references to Dienes blocks without linking them to the arithmetic operation and current column step. Use student\_id exclusively (1-12). Bind Socratic persona exclusively to digital VRA model representations, strictly forbidding direct answer disclosure or physical CRA terminology. Serve pre-configured static hints instantly upon API timeout or network failure. The AI must return error\_category ('calculation' | 'procedural' | 'conceptual') in every response; treat a response lacking it as malformed and fall back to the static hint. Persist error\_category into the SOCRATIC\_CARD\_SHOWN telemetry event. Never discard the classification after rendering the card — it is the sole data source for the radar cognitive signal (Module 18\) and the report analysis (Module 23).*
...
**שלושה מצבי מפגש בשליטת המורה (החלטת בעל המוצר, 6 בספטמבר 2026):** לכל מפגש עומדות למורה שלוש פעולות: הפעלה, עצירה וסגירה. עצירה משאירה את המפגש פתוח ומציגה לכל הלומדים, במקום ובזמן אמת, מסך המתנה שקט ("המורה עצרה את הפעילות לרגע"), כשמרחב העבודה שמור מתחתיו ללא שינוי; "המשך מפגש" מחזיר את הלומדים לאותה נקודה. סגירה מציגה לכל הלומדים, במקום ובזמן אמת, מסך "המורה סגרה את המפגש", ללא ניווט אוטומטי ללובי. כל מעבר מצב מגיע ללומד באמצעות המאזין בזמן אמת, ובנוסף בקריאה יזומה מהשרת מדי 15 שניות ובכל חזרה לחלון, כדי שלומד שהמאזין שלו נותק בשקט יתעדכן בכל מקרה.
...
**גבול עליון של 45 דקות (החלטת בעל המוצר, 6 בספטמבר 2026):** מפגש שהמורה לא סגרה נסגר מעצמו 45 דקות לאחר ההפעלה, ולא לפני כן. עד אז שום דבר אינו נסגר מעצמו, והלומד אינו רואה שעון או הודעה. הסגירה האוטומטית מתועדת ברשומת המפגש (endedBy: auto\_45min), והלומדים רואים אותה כסגירה רגילה, במקום.
...
**הערת מימוש (Implementation Note):** הפער המתואר להלן נמצא בשכבת התשתית בלבד ואינו משנה את ההתנהגות הפדגוגית, את חוויית הלומד או את מדיניות האנונימיות (Zero-PII). כל יעדי הביצועים במסמך מתקיימים. סטטוס: מתועד ומאושר לפיילוט. בפועל, המצב ממומש ב-Realtime Database תחת system\_control/projector\_mode, כולל אימות חותמות זמן למניעת עדכונים לא מסודרים, כאשר יעד הביצועים P95 ≤ 1000ms מתקיים.
...
* **כחול (Blue):** התלמיד לחץ על כפתור קריאה לעזרה (helpRequested \= true).

* **אפור (Grey):** התלמיד מנותק (לפי Presence Heartbeat) או שטרם החל סשן.

**היררכיית עדיפות תצוגה (Priority Hierarchy), מהגבוה לנמוך:** BLUE \> RED \> GREY \> YELLOW \> GREEN. העדכון מבוצע בתוך עד שנייה אחת באמצעות Firestore Snapshot Listener.
...
*Implement Silent Radar as a static 3x4 grid representing 12 anonymous student profiles (IDs 1-12). Throttle client writes to maximum once per 1000ms. Apply color precedence highest to lowest: BLUE \> RED \> GREY \> YELLOW \> GREEN, updated within 1 second using a Firestore Snapshot Listener. Server-side Presence Heartbeat determines GREY state within a 15-second max target. Strictly forbid rendering names, initials, or avatars. Render a small static cognitive glyph in each grid cell derived from the learner's most recent error\_category value: ח for calculation, ר for procedural, מ for conceptual, and nothing when no classification exists. The glyph must not blink, animate, move, or alter cell dimensions. Clicking a cell opens a detail layer showing that learner's classification distribution for the current session. The background colours remain driven strictly by activity timing, connection state, and help requests; the glyph is an independent layer and must never override or recolour the cell.*

**הערת מימוש (Implementation Note):** הפער המתואר להלן נמצא בשכבת התשתית בלבד ואינו משנה את ההתנהגות הפדגוגית, את חוויית הלומד או את מדיניות האנונימיות (Zero-PII). כל יעדי הביצועים במסמך מתקיימים. סטטוס: מתועד ומאושר לפיילוט. בפועל, הרכיב ממומש באמצעות מאזין Realtime Database על users/students, עם ויסות כתיבות של 1000ms כנדרש. המימוש ב-Firestore נמנע מכיוון שהיה מייצר עלות קריאות גבוהה מאוד ללא רווח תפקודי.
...
מחולל הדוחות פועל בצד השרת בלבד באמצעות Cloud Function (generatePedagogicalReportPDF). המחולל מופעל על ידי המורה, לכל מפגש (1 עד 8) של כל לומד, מתוך "דו"חות אבחון אישיים" (החלטת בעל המוצר, 4 בספטמבר 2026; במקור: עם השלמת מפגש 2 או 8 בלבד), ומפיק קובץ PDF לקריאה בלבד השמור ב-Cloud Storage ומקושר למסמך המורה; תוכן הדוח מוצג גם באותו עמוד. הפונקציה קוראת את session\_score\_percent הקיים כבר על SessionDocument (המחושב בטריגר עצמאי על סיום מפגש); למפגש שאין לו SessionDocument היא מחשבת את הציון מאירועי הטלמטריה של אותו מפגש לפי הנוסחה שלהלן, כאשר המכנה הוא מספר תרגילי החובה של אותו מפגש (7 במפגש האבחון).
...
**דוח כיתה לכל מפגש (החלטת בעל המוצר, 6 בספטמבר 2026):** לצד דוח התלמיד, המורה מפיקה מאותה לשונית דוח כיתתי לכל מפגש (1 עד 8) באמצעות Cloud Function נפרדת (generateClassMeetingReport). הדוח מודד לכל לומד את כל מה שנמדד בדוח האישי, מאותם אירועי טלמטריה ובאותן פונקציות חישוב (ציון ניסיון ראשון לפי הנוסחה שלעיל, קבוצת העבודה לפי כלל האחוזים, ספרות שגויות לפי טור, מחיקות, ביטולים, היסוסים, המרות, כרטיסי חניכה וסיווגיהם, תוצאת כל תרגיל, זמן פעילות, הקלטה ורפלקציה), ומצרף ברמת הכיתה: ממוצע, חציון, טווח, חלוקה לשלוש קבוצות העבודה, הצלחה בניסיון ראשון לכל תרגיל, טעויות לפי טור והתפלגות הטריגרים. שכבת הניתוח של מנוע הבינה פועלת גם כאן, ברמת הכיתה בלבד (דפוסים כיתתיים והמלצות הוראה לכיתה), ואסור לה לשנות את חלוקת הלומדים לקבוצות; בכשל מוצג אותו טקסט קבוע. הפלט: PDF למורה, CSV של טבלת הלומדים למחקר, ומסמך class\_reports ב-Firestore הנקרא על ידי מורת הכיתה בלבד ונכתב על ידי השרת בלבד.
...
**הערת מימוש (Implementation Note):** הפער המתואר להלן נמצא בשכבת התשתית בלבד ואינו משנה את ההתנהגות הפדגוגית, את חוויית הלומד או את מדיניות האנונימיות (Zero-PII). כל יעדי הביצועים במסמך מתקיימים. סטטוס: מתועד ומאושר לפיילוט. בפועל, שידור המפגש הפעיל לתלמידים ומנגנון חלון החסד לניתוק המורה ממומשים ב-Realtime Database תחת הצומת active\_class\_session, וזאת בשל זמן תגובה נמוך יותר ועלות קריאות נמוכה משמעותית בשידור רציף ל-12 לומדים במקביל, בעוד העדכון האטומי ל-ClassDocument ולמסמכי הלומדים אכן מתבצע ב-Firestore כנדרש.
...
2. **איפוס לומד יחיד (Single Student Reset):** מוחק את מצב מרחב העבודה ואת התקדמות המפגש הפעיל של לומד בודד בלבד, ומחזיר אותו לתחילת המפגש. מחייב גיבוי מלא לפני הביצוע. בחלון האישור המורה בוחרת במפורש בין **המפגש הפעיל בלבד** (ברירת המחדל: מפגשים קודמים, הקלטות והודעות צ'אט נשמרים) לבין **איפוס מוחלט של הלומד** (כל 8 המפגשים, תוצאות האבחון וההקלטות נמחקים). הבחירה נרשמת ביומן הביקורת.
...
קטלוג תוכנית הלימודים מנוהל כערך קנוני ב-Cloud Firestore, המאפשר עדכונים אסינכרוניים ודינמיים של תבניות התרגילים. בצד הלקוח, הקטלוג נטען מ-Firestore ונשמר במטמון מקומי ב-IndexedDB, מה שמבטיח טעינה מהירה ועמידות מלאה במצב לא מקוון (Offline Resilience). הקטלוג מנהל את 7 תרגילי החובה ואת משימות הבחירה (ביסוס מול אתגר).
...
4. **useWorkspaceStore:** מנהל את מצב מרחב העבודה, הבלוקים, הטורים, המקלדת, ומשתנה המצב helpRequested המאפשר מיתוג דו-כיווני לקריאת עזרה.
...
**הערת מימוש (Implementation Note):** הפער המתואר להלן נמצא בשכבת התשתית בלבד ואינו משנה את ההתנהגות הפדגוגית, את חוויית הלומד או את מדיניות האנונימיות (Zero-PII). כל יעדי הביצועים במסמך מתקיימים. סטטוס: מתועד ומאושר לפיילוט. בפועל מנוע התרגילים פועל על טיפוס פנימי בשם SessionTask, המכיל את אותם שדות פדגוגיים בשמות שונים, וזאת כדי לשמור על מבנה קטלוג תוכנית הלימודים ב-Firestore.
...
# **נספח ב': מרשם פערי מימוש מתועדים**

| מודול | הדרישה במסמך | המימוש בפועל | נימוק | סטטוס |
| :---- | :---- | :---- | :---- | :---- |
| מודול 14 | עדכון active\_session\_id ב-Cloud Firestore | שידור המפגש ממומש ב-Realtime Database (active\_class\_session) | זמן תגובה נמוך ועלות קריאות מופחתת | מתועד ומאושר לפיילוט |
| מודול 15 | סנכרון באמצעות מאזין Firestore | מימוש ב-Realtime Database (system\_control/projector\_mode) | עמידה ביעד ביצועים P95 ≤ 1000ms | מתועד ומאושר לפיילוט |
| מודול 18 | Firestore Snapshot Listener | מאזין Realtime Database עם ויסות כתיבות של 1000ms | מניעת עלות קריאות גבוהה | מתועד ומאושר לפיילוט |
| נספח א' | שימוש בטיפוס ExerciseTemplate | שימוש בטיפוס פנימי SessionTask | תאימות למבנה קטלוג תוכנית הלימודים | מתועד ומאושר לפיילוט |
