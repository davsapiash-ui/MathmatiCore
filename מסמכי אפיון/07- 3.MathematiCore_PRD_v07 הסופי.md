©מתמטיקאור | MathematiCOre
מסמך דרישות מוצר -PRD

גרסה 7.3 | עודכן 18 בספטמבר 2026, 17:37
...
**מדדי המחקר.** המערכת מחשבת בצד השרת ארבעה מדדים לכל לומד ולכל מפגש, מתוך אירועי הטלמטריה בלבד. המדדים מוצגים בדוח הלומד ובדוח הכיתה, ואינם מוצגים ללומד. 

* **מדד 1, עצמאות מושגית:** session\_score\_percent, כמוגדר לעיל.

* **מדד 2, התמדה וויסות עצמי:** Persistence Index, כמוגדר במודול 16\.

* **מדד 3, גמישות ייצוגית:** Flexibility Index \= (R ÷ T) × 100\. **T** \= מספר תרגילי החובה שה-operation שלהם הוא representation ושהלומד השלים במפגש. **R** \= מספרם מתוך אלה שהושלמו בניסיון ראשון, כלומר אירוע PROBLEM\_COMPLETE שבו error\_count \=== 0\. בתרגילי ייצוג, error\_count סופר כל בדיקת לוח שנכשלה. המדד מחושב במפגשים 3 ו-7. בנוסף מחושב ערך מצטבר ללומד: סך R בשני המפגשים חלקי סך T בשני המפגשים. כאשר T \= 0 המדד אינו מחושב וערכו null.

* **מדד 4, אפקטיביות התיווך:** Mediation Effectiveness \= (S ÷ C) × 100\. **C** \= מספר אירועי SOCRATIC\_CARD\_SHOWN במפגש. **S** \= מספר הכרטיסים שאחריהם התשובה הבאה של הלומד באותו exercise\_id הייתה נכונה: אירוע DIGIT\_ENTERED הראשון שבו is\_correct אינו null הוא is\_correct \=== true, ובתרגיל ללא הקלדת ספרות, אירוע PROBLEM\_COMPLETE שהגיע לפני כרטיס חניכה נוסף. בנוסף מחושב ערך מצטבר ללומד: סך S בכל המפגשים חלקי סך C בכל המפגשים. לצד האחוז מוצג תמיד גם הערך C עצמו. כאשר C \= 0 האחוז אינו מחושב וערכו null, והלומד מסומן "לא נדרש תיווך". דוח הכיתה מציג לכל מפגש את מספר הלומדים שלא נדרשו לתיווך מתוך 12\. במפגש 2 כרטיס החניכה מנוטרל, ולכן המדד אינו מחושב בו.

מדד 5, השפעת האנליטיקה על תיווך המורה, בוחן את המורה החוקרת עצמה. הוא נמדד ביומן החוקר ואינו מחושב במערכת.
...
2. **איפוס מפגש (Session Reset):** מוחק את מצב מרחב העבודה ואת התקדמות המפגש הפעיל, ומחזיר לתחילת המפגש. לפעולה שני יעדים: לומד בודד, או כל הכיתה בפעולה אחת. ביעד "לומד בודד" המורה רשאית לבחור במקום זאת באיפוס מוחלט של אותו לומד. ביעד "כל הכיתה" מתאפס המפגש הפעיל בלבד, והפעולה אפשרית רק כשמפגש פתוח לכיתה; מחיקת כלל נתוני הכיתה היא רמה 3 בלבד. מחייב גיבוי מלא לפני הביצוע.
