...
// 1. Remove basicAdditionGroup to pendingRouteStudents
let startIndex = lines.findIndex(line => line.includes('const basicAdditionGroup = allStudents.filter('));
let endIndex = lines.findIndex(line => line.includes('const pendingRouteStudents ='));
if (startIndex !== -1 && endIndex !== -1) {
  lines.splice(startIndex, endIndex - startIndex);
}

// 2. Remove qMatrixData
startIndex = lines.findIndex(line => line.includes('const qMatrixData = useMemo(() => {'));
endIndex = lines.findIndex(line => line.includes('// Generate trace data alerts'));
// wait, qMatrixData ends before alerts. We want to remove until the line before // Generate trace data alerts
if (startIndex !== -1 && endIndex !== -1) {
  // we actually just want to remove the qMatrixData block.
  lines.splice(startIndex, endIndex - startIndex - 1);
}

// 3. Replace the main return block
startIndex = lines.findIndex(line => line === '  return (');
if (startIndex !== -1) {
