...
export function VisualGraphicOrganizer({ isSubtraction }: { isSubtraction?: boolean }) {
  ...
  return (
    <div className="flex w-full justify-between items-start bg-white/60 dark:bg-slate-900/60 p-4 rounded-2xl border-2 border-ws-blue/20 shadow-sm relative overflow-hidden">
      ...
      {steps.map((step, idx) => (
        <motion.div 
          ...
          className="relative z-10 flex flex-col items-center gap-2 bg-white dark:bg-slate-800 px-2 py-2 rounded-xl shadow-sm border border-ws-surface2 w-[30%]"
        >
