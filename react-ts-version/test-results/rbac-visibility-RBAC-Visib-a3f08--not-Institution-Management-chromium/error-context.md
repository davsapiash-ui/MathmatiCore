...
- Name: rbac-visibility.spec.ts >> RBAC Visibility Tests >> Teacher has access to Class Management but not Institution Management
...
Call log:
  ...
  - waiting for getByText('מיפוי כיתתי')
...
> 33 |     await expect(page.getByText('מיפוי כיתתי')).toBeVisible();
     |                                                 ^ Error: expect(locator).toBeVisible() failed
