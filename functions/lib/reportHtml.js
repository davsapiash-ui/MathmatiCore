...
exports.EXACT_AI_FALLBACK_TEXT_HE = "הניתוח הפדגוגי המפורט אינו זמין כעת. ההמלצות שלהלן מבוססות על מדדי הביצוע.";
exports.TIER_LABEL_HE = {
...
function embeddedFontCss() {
    if (fontCss !== null)
        return fontCss;
    const candidates = [(0, htmlPdf_1.fontPath)("Heebo.ttf"), (0, htmlPdf_1.fontPath)("Arial.ttf")];
...
function reportFooterTemplate(generatedAt) {
...
const asStringArray = (value) => Array.isArray(value) ? value.map((v) => String(v !== null && v !== void 0 ? v : "")).filter((v) => v.length > 0) : [];
// ---------------------------------------------------------------------------
// Individual pedagogical report (one learner, one meeting).
// ---------------------------------------------------------------------------
function pedagogicalReportHtml(report) {
    ...
    if (gaps.length > 0 || teaching.length > 0) {
        insights = "";
        if (gaps.length > 0)
            insights += `<h3>פערי ידע שאותרו:</h3>${bulletList(gaps, "amber-text")}`;
        if (teaching.length > 0)
            insights += `<h3>המלצות הוראה להמשך העבודה בכיתה:</h3>${bulletList(teaching, "amber-text")}`;
    }
    else {
...
function outcomesTable(rows, exerciseIds) {
    ...
    const body = rows.map((r) => {
        const cells = exerciseIds.map((id) => {
            ...
            return `<td class="${cls}">${outcome ? esc(exports.OUTCOME_HE[outcome]) : "לא נפתח"}</td>`;
        }).join("");
...
exports.CLASS_REPORT_PDF_OPTIONS = { landscape: true };
function classReportHtml(report) {
    const title = report.title_he || "MathematiCore - דוח כיתה";
    const a = report.aggregates;
    const rows = Array.isArray(report.learners) ? report.learners : [];
    ...
    const tiers = ["below_50", "between_50_75", "above_75"]
        .map((tier) => `<li class="green-text"><b>${esc(exports.TIER_LABEL_HE[tier])}:</b> ${esc(studentList(a.tiers[tier]))}</li>`)
        .join("");
    ...
    if (patterns.length > 0 || teaching.length > 0) {
        ...
        if (patterns.length > 0)
            analysis += `<h3>דפוסים כיתתיים שאותרו:</h3>${bulletList(patterns, "amber-text")}`;
        if (teaching.length > 0)
            analysis += `<h3>המלצות הוראה לכיתה:</h3>${bulletList(teaching, "amber-text")}`;
    }
    else {
