...
function createClassReportPdfBuffer(report) {
    return new Promise((resolve, reject) => {
        try {
            ...
            const line = (text, size = 10, color = "#334155", maxChars = 80) => {
                doc.fontSize(size).fillColor(color).text((0, pedagogicalReport_1.wrapAndBidi)(text, maxChars), { align: "right", lineGap: 3 });
            };
            const heading = (text, color = "#1e293b") => {
                doc.moveDown(0.8);
                doc.fontSize(14).fillColor(color).text((0, pedagogicalReport_1.shapeRtl)(text), { align: "right" });
                doc.moveDown(0.3);
            };
            doc.fontSize(22).fillColor("#1e1b4b").text((0, pedagogicalReport_1.shapeRtl)(report.title_he), { align: "center" });
            doc.moveDown(0.4);
            doc.fontSize(11).fillColor("#475569").text((0, pedagogicalReport_1.shapeRtl)("דוח כיתתי חסוי | מדיניות אפס מידע מזהה (Zero PII) | לומדים מזוהים במספר בלבד"), { align: "center" });
            doc.moveDown(1);
            doc.rect(40, doc.y, 515, 60).fillAndStroke("#f8fafc", "#cbd5e1");
            doc.fillColor("#0f172a").fontSize(11);
            const cardY = doc.y + 12;
            doc.text((0, pedagogicalReport_1.shapeRtl)(`מפגש: ${report.session_number}`), 400, cardY, { width: 140, align: "right" });
            doc.text((0, pedagogicalReport_1.shapeRtl)(`לומדים עם נתונים: ${a.learners_with_data} מתוך 12`), 200, cardY, { width: 190, align: "right" });
            doc.text((0, pedagogicalReport_1.shapeRtl)(`ציון ממוצע: ${a.score_mean}%`), 55, cardY, { width: 140, align: "right" });
            doc.text((0, pedagogicalReport_1.shapeRtl)(`חציון: ${a.score_median}% | טווח: ${a.score_min}%–${a.score_max}% | מסלול ירוק: ${a.paths.green_path} | מסלול ביסוס: ${a.paths.remediation_path}`), 55, cardY + 25, { width: 490, align: "right" });
            doc.y = cardY + 60;
            ...
            doc.fontSize(8).fillColor("#94a3b8").text((0, pedagogicalReport_1.shapeRtl)(`נוצר אוטומטית בתאריך ${genTime.toLocaleDateString("he-IL")} | מנוע MathematiCore v7.0`), { align: "center" });
            doc.end();
        }
        catch (err) {
            reject(err);
