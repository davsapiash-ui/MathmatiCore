...
function createPedagogicalReportPdfBufferWithPdfkit(report) {
    return new Promise((resolve, reject) => {
        try {
            ...
            (0, hebrewPdf_1.rtlText)(doc, report.title_he || "MathematiCore - דוח פדגוגי מסכם", { align: "center" });
            doc.moveDown(0.4);
            doc.fontSize(11).fillColor("#475569");
            (0, hebrewPdf_1.rtlText)(doc, "הערכה פדגוגית חסויה | מדיניות אפס מידע מזהה (Zero PII)", { align: "center" });
            doc.moveDown(1);
            // Metadata summary card
            doc.rect(40, doc.y, 515, 60).fillAndStroke("#f8fafc", "#cbd5e1");
            doc.fillColor("#0f172a").fontSize(11);
            const cardY = doc.y + 12;
            (0, hebrewPdf_1.rtlText)(doc, `לומד: ${report.anonymous_student_label}`, 390, cardY, { width: 150 });
            (0, hebrewPdf_1.rtlText)(doc, `מפגש: ${report.session_number}`, 260, cardY, { width: 110 });
            (0, hebrewPdf_1.rtlText)(doc, `ציון שליטה: ${report.score_percent}%`, 70, cardY, { width: 170 });
            (0, hebrewPdf_1.rtlText)(doc, `מסלול מומלץ: ${report.matrix_recommended_path === 'green_path' ? 'מסלול העמקה (ירוק)' : 'מסלול ביסוס ומענה מותאם (צהוב)'}`, 55, cardY + 25, { width: 490 });
            doc.x = 40;
            doc.y = cardY + 60;
            doc.moveDown(1);
            // Grouping Recommendation
            doc.fontSize(14).fillColor("#166534");
            (0, hebrewPdf_1.rtlText)(doc, "1. המלצת ניתוב פדגוגי");
            doc.moveDown(0.3);
            doc.fontSize(11).fillColor("#14532d");
            (0, hebrewPdf_1.rtlText)(doc, `קבוצת למידה: ${report.routing_label_he || report.routing_group}`);
            doc.fontSize(10).fillColor("#334155");
            (0, hebrewPdf_1.rtlText)(doc, `פירוט פדגוגי: ${report.recommendation_details_he || report.routing_label_he}`, { lineGap: 3 });
            doc.moveDown(1);
            // Chronological Exercise Narratives
            doc.fontSize(14).fillColor("#1e293b");
            (0, hebrewPdf_1.rtlText)(doc, "2. סיפור התרגילים הכרונולוגי (Exercise Narratives)");
            doc.moveDown(0.4);
            if (report.exercise_narratives && Array.isArray(report.exercise_narratives)) {
                for (const narrative of report.exercise_narratives) {
                    doc.fontSize(10).fillColor("#334155");
                    (0, hebrewPdf_1.rtlText)(doc, `• ${narrative}`, { lineGap: 3 });
                    doc.moveDown(0.3);
                }
            }
            doc.moveDown(1);
            ...
            (0, hebrewPdf_1.rtlText)(doc, "3. תובנות קוגניטיביות פדגוגיות");
            doc.moveDown(0.3);
            ...
            if (gaps.length > 0 || teaching.length > 0) {
                if (gaps.length > 0) {
                    ...
                    (0, hebrewPdf_1.rtlText)(doc, "פערי ידע שאותרו:");
                    doc.moveDown(0.2);
                    for (const gap of gaps) {
                        doc.fontSize(10).fillColor("#78350f");
                        (0, hebrewPdf_1.rtlText)(doc, `• ${gap}`, { lineGap: 3 });
                        doc.moveDown(0.2);
                    }
                    doc.moveDown(0.4);
                }
                if (teaching.length > 0) {
                    doc.fontSize(11).fillColor("#92400e");
                    (0, hebrewPdf_1.rtlText)(doc, "המלצות הוראה להמשך העבודה בכיתה:");
                    doc.moveDown(0.2);
                    for (const rec of teaching) {
                        doc.fontSize(10).fillColor("#78350f");
                        (0, hebrewPdf_1.rtlText)(doc, `• ${rec}`, { lineGap: 3 });
                        doc.moveDown(0.2);
                    }
                }
            }
                ...
                doc.fontSize(10).fillColor("#78350f");
                (0, hebrewPdf_1.rtlText)(doc, report.ai_fallback_text || exports.EXACT_AI_FALLBACK_TEXT, { lineGap: 3 });
            }
            doc.moveDown(1.5);
            // Footer
            ...
            (0, hebrewPdf_1.rtlText)(doc, footerText, 40, Math.max(doc.y + 10, 780), { width: 515, align: "center" });
            doc.end();
        }
        catch (renderErr) {
            reject(renderErr);
...
exports.generatePedagogicalReportPDF = (0, https_1.onCall)(Object.assign(Object.assign({}, geminiConfig_1.GEMINI_SECRETS), htmlPdf_1.CHROMIUM_PDF_RUNTIME), async (request) => {
    ...
    const studentDigits = String((_a = explicitStudentId !== null && explicitStudentId !== void 0 ? explicitStudentId : sessionId.split("_").pop()) !== null && _a !== void 0 ? _a : "").replace(/\D/g, "");
    if (!studentDigits) {
        throw new https_1.HttpsError("invalid-argument", "Missing studentId (1-12).");
    }
    ...
    const aliasSnaps = await Promise.all([`student_user${clampedStudentNum}`, `student_${clampedStudentNum}`, `${clampedStudentNum}`].map((alias) => rtdb.ref(`users/students/${alias}`).get()));
    const studentVal = ((_c = aliasSnaps.find((snap) => snap.exists())) === null || _c === void 0 ? void 0 : _c.val()) || {};
    let sessionData;
    let scoreSource;
    if (sessionDoc.exists && ((_d = sessionDoc.data()) === null || _d === void 0 ? void 0 : _d.session_score_percent) !== undefined && ((_e = sessionDoc.data()) === null || _e === void 0 ? void 0 : _e.session_score_percent) !== null) {
            ...
            try {
                ...
                for (const bankId of bankIds) {
                    ...
                    if (Array.isArray(tasks) && tasks.length > 0) {
                        compulsoryTotal = tasks.filter((t) => t && t.isOptionalChoiceTask !== true).length || tasks.length;
                        break;
                    }
                }
            }
        ...
        throw new https_1.HttpsError("not-found", `אין פעולות מתועדות למפגש ${resolvedSessionNumber} של תלמיד ${clampedStudentNum}; אין מה לנתח.`);
    }
    ...
    for (const doc of telemetryDocs) {
        ...
        const isWrongDigit = doc.event_type === "DIGIT_ENTERED" &&
            ((_f = doc.details) === null || _f === void 0 ? void 0 : _f.is_correct) === false;
    ...
    if (minTimestamp !== Infinity && maxTimestamp !== -Infinity) {
        dataRangeStr = `${(0, exportDriveReport_1.formatIsraelDateTime)(minTimestamp)} — ${(0, exportDriveReport_1.formatIsraelDateTime)(maxTimestamp)}`;
    }
    ...
    const provenance = {
        ...
        generatedAtIsrael: (0, exportDriveReport_1.formatIsraelDateTime)(generatedAt),
        ...
        aiLayerStatus: Boolean(aiAnalysis)
            ...
            : exports.EXACT_AI_FALLBACK_TEXT,
    };
    ...
    try {
        ...
        try {
            const driveFileName = (0, exportDriveReport_1.buildDriveFileName)({
                kind: "learner_report",
