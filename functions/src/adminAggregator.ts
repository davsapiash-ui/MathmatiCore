...
export const hourlyAdminAggregator = onSchedule({
  ...
  const perSession: Record<string, { created: number; completed: number; scoreSum: number }> = {};
  ...
  const sessionBreakdown = Object.entries(perSession).reduce<Record<string, {
    created: number; completed: number; completion_rate_percent: number; average_score_percent: number;
  }>>((acc, [sessionNumber, agg]) => {
    acc[sessionNumber] = {
      created: agg.created,
